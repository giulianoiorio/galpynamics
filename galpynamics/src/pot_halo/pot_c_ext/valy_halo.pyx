...
cdef double integrand_valy(int n, double *data) nogil:
    ...
    if (m<=mcut): psi=psi_valy(d0,rb,m)
    else: psi=psi_valy(d0,rb,mcut)
...
cdef double  _potential_valy(double R, double Z, double mcut, double d0, double rb, double e, double toll):
    ...
    #Integ
    import galpynamics.src.pot_halo.pot_c_ext.valy_halo as mod
    fintegrand=LowLevelCallable.from_cython(mod,'integrand_valy')

    intpot=quad(fintegrand,0.,m0,args=(R,Z,mcut,d0,rb,e),epsabs=toll,epsrel=toll)[0]


    psi=psi_valy(d0,rb,mcut)
...
cpdef potential_valy(R, Z, d0, rb, e, mcut, toll=1e-4, grid=False):

    if isinstance(R, float) or isinstance(R, int):
        if isinstance(Z, float) or isinstance(Z, int):
            return np.array(_potential_valy(R=R,Z=Z,mcut=mcut,d0=d0,rb=rb,e=e,toll=toll))
        else:
        ...
        if grid:
            ...
            return np.array(_potential_valy_grid( R=R, Z=Z, nlenR=len(R), nlenZ=len(Z), mcut=mcut, d0=d0, rb=rb, e=e, toll=toll))
        elif len(R)==len(Z):
            nlen=len(R)
            R=np.array(R,dtype=np.dtype("d"))
            Z=np.array(Z,dtype=np.dtype("d"))
            return np.array(_potential_valy_array( R=R, Z=Z, nlen=len(R), mcut=mcut, d0=d0, rb=rb, e=e, toll=toll))
        else:
            raise ValueError('R and Z have different dimension')
...
cdef double _vcirc_valy(double R, double d0, double rb, double e, double toll):
    """
    ...
    #Integ
    import galpynamics.src.pot_halo.pot_c_ext.valy_halo as mod
    fintegrand=LowLevelCallable.from_cython(mod,'vcirc_integrand_valy')
...
cdef double[:,:] _vcirc_valy_array(double[:] R, int nlen, double d0, double rb, double e, double toll):
    """
    Calculate Vcirc on a single point on the plane
    :param R: radii array (kpc)
    :param d0: Central density (Msol/kpc^3)
...
cpdef vcirc_valy(R, d0, rb, e, toll=1e-4):
    ...
    if isinstance(R, float) or isinstance(R, int):
        ...
        if R==0: ret=0
        else: ret= _vcirc_valy(R, d0, rb,  e,  toll)

    else:

        ret=_vcirc_valy_array(R, len(R), d0, rb, e, toll)
